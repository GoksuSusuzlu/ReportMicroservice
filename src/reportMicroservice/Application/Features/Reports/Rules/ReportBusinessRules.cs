...
public class ReportBusinessRules : BaseBusinessRules
{
    ...
    public async Task ReportShouldHaveValidStatusCode(string statuscode)
    {
        if (statuscode != "PREPARING" && statuscode != "COMPLETED")
        {
            await throwBusinessException(ReportsBusinessMessages.InvalidStatusCode);
        }
    }
}